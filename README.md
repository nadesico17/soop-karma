...
import { 
  ...
  Wifi, WifiOff, ArrowRight, Share2, Info
} from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const systemAppId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
    setAppId(systemAppId);
    setDb(firestore);
  ...
  useEffect(() => {
    if (!user || !db || !channelId || !appId) return;

    // Karmas
    const karmasRef = collection(db, 'artifacts', appId, 'public', 'data', 'karmas');
    ...
    // Settings
    const settingsRef = collection(db, 'artifacts', appId, 'public', 'data', 'channel_settings');
    const unsubSettings = onSnapshot(settingsRef, (snapshot) => {
        ...
        if (mySettings) {
            ...
            setStreamerName(channelId.toUpperCase());
  ...
  const updateSettings = async (field, value) => {
    ...
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'channel_settings', channelId), { 
      [field]: value 
    }, { merge: true });
  };
  ...
  const addKarma = async (e) => {
    ...
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'karmas'), {
      channelId: channelId,
      title: newKarma,
  ...
  const deleteKarma = async (id) => {
    if (!db || !user) return;
    await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'karmas', id));
  };

  const toggleComplete = async (item) => {
    if (!isAdmin || !db || !user) return;
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'karmas', item.id), {
  ...
  const saveEdit = async (id) => {
    ...
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'karmas', id), {
  ...
  const addViewer = async (e) => {
    ...
    if (!viewers.some(v => v.name === newViewerName.trim())) {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'viewers'), {
        channelId: channelId,
  ...
  const confirmDeleteViewer = async () => {
    ...
    if (viewerId) {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'viewers', viewerId));
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans">
        ...
        <div className="mt-8 text-xs text-slate-400">
           Free Service for Streamers • Realtime Sync
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col md:flex-row relative">
      <aside className="w-full md:w-64 bg-white border-b md:border-r border-slate-200 flex flex-col flex-shrink-0 z-10">
        <div className="p-4 border-b border-slate-100 bg-gradient-to-r from-indigo-500 to-purple-600 text-white flex justify-between items-center">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-xs opacity-80">{isOnline ? '실시간 연동 중' : '오프라인'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-100 mt-auto hidden md:flex justify-between items-center bg-slate-50">
           <span className="text-xs text-slate-400 font-medium">관리자 모드</span>
           <button 
          ...
          <div className="w-full max-w-4xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 relative">
                ...
                <div>
                  <div className="flex items-center gap-2 h-10">
                    {isAdmin && isEditingName ? (
                      ...
                      >
                        {streamerName || channelId} <span className="opacity-80 font-light">|</span> Karma
                  ...
                  <p className="opacity-90 mt-1 flex items-center gap-2">
                     {selectedViewer === 'All' ? '전체 업보 현황' : `User: ${selectedViewer}님의 업보`}
                  </p>
                </div>
                <div className="hidden md:flex bg-white/20 backdrop-blur-md px-4 py-2 rounded-2xl flex-col items-center min-w-[80px]">
                  <span className="text-xs font-semibold uppercase tracking-wider opacity-80">Progress</span>
            ...
            <div className="flex border-b border-slate-100">
              {['All', 'Active', 'Completed'].map((tab) => (
                ...
                >
                  {tab === 'All' ? '전체 보기' : tab === 'Active' ? '진행 중 🔥' : '청산 완료 ✅'}
            ...
            <div className="p-6 bg-white min-h-[400px]">
              {!isOnline && (
                 <div className="bg-red-50 text-red-500 p-4 rounded-xl mb-4 flex items-center gap-2 text-sm font-bold justify-center">
                   <WifiOff size={18}/> 서버와 연결이 끊어졌습니다. 인터넷 연결을 확인해주세요.
                 </div>
              )}
              {finalDisplayKarmas.length === 0 ? (
                <div className="h-64 flex flex-col items-center justify-center text-slate-400">
                  <Trophy size={48} className="mb-4 text-slate-200" />
                  <p>{selectedViewer === 'All' ? '표시할 업보가 없습니다.' : `'${selectedViewer}'님의 업보가 없습니다.`}</p>
                </div>
              ) : (
                        ...
                        <div className="flex items-center w-full">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              ...
                              {viewerUrl ? (
                                <a 
                                  ...
                                  title={`${viewerUrl}로 이동`}
                                >
                                   <User size={10} /> {item.viewer || '익명'} <ExternalLink size={8} className="ml-0.5 opacity-50"/>
                                </a>
                              ) : (
                          ...
                          {isAdmin && (
                            <div className="flex gap-1 ml-auto md:ml-0 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                               <button onClick={() => startEditing(item)} className="p-2 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-lg"><Edit2 size={18} /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl scale-100 animate-in zoom-in-95">
            ...
            <p className="text-slate-600 mb-6">
              정말로 <span className="font-bold text-purple-600">{deleteModal.viewerName}</span>님을 목록에서 삭제하시겠습니까?<br/>
              <span className="text-xs text-slate-400">(등록된 업보 데이터는 유지됩니다.)</span>
            </p>
